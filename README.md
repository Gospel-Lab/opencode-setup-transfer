...
지금 쓰는 컴퓨터의 AI 도구 세팅을 **파일 하나로 포장해서**, 새 컴퓨터에서 **자동으로 풀어놓는** 도구입니다.

**opencode · Claude Code · codex** 를 지원합니다. 실행하면 그 컴퓨터에 깔려 있는 것만 알아서 찾아 담습니다.
...
쓰시던 AI 도구(opencode·Claude Code·codex)를 열고 아래를 그대로 붙여넣으세요.
설치 위치와 절차는 저장소 안 [AGENTS.md](AGENTS.md) 에 적혀 있어서, AI 가 알아서 따라갑니다.

```
아래 저장소를 내 컴퓨터에 설치해줘. AGENTS.md 의 지시를 그대로 따라줘.
https://github.com/Gospel-Lab/ai-setup-transfer
```

설치가 끝나면 **AI 도구를 껐다 켜야** 새 기능이 인식됩니다.

### 직접 하는 방법 — 명령 한 줄

터미널에 익숙하시면 이쪽이 더 빠릅니다.
...
3. Firebase·Supabase 는 **프로젝트 만드는 법**까지 단계별로 안내합니다 (로그인만으로는 쓸 수 없습니다)
4. 로그인할 때 **화면에 무엇이 뜨고 어디를 눌러야 하는지** 알려줍니다
5. 끝나면 **정말 연결됐는지 자동으로 확인**하고, 안 됐으면 원인을 짚어줍니다
6. 이미 연결된 서비스는 **건너뜁니다**. 안 쓰는 서비스도 건너뛸 수 있습니다
...
| 도구 | 설정 폴더 | 담는 것 |
|---|---|---|
| opencode | `~/.config/opencode` | 설정, 스킬·명령·에이전트·테마·모드·플러그인, 전역 `AGENTS.md` |
| Claude Code | `~/.claude` | `settings.json`, 스킬·명령·훅·스크립트, 상태줄, 플러그인 목록, `CLAUDE.md` |
| codex | `~/.codex` | `config.toml`, 스킬, 프롬프트, `AGENTS.md` |

설정 파일의 키 값은 자리표시자로 비우고, 주석은 그대로 살립니다.

**안 옮기는 것**: 각 도구의 인증 파일(API 키), 대화 기록·세션·로그·캐시, `node_modules`·플러그인 캐시, GitHub·Firebase·Vercel 등의 토큰. 특히 대화 기록은 도구에 따라 1GB가 넘습니다.
...
- **심볼릭 링크 실체화** — opencode 는 링크된 스킬 폴더를 따라가지 않으므로 내용을 복사합니다. 5MB 초과 도구와 순환 링크는 제외하고 `SYMLINKS.md` 에 이유와 함께 기록합니다.
- **두 번 실행해도 안전** — 같은 아카이브를 다시 적용해도 결과가 달라지지 않고, 매번 백업이 남습니다.
